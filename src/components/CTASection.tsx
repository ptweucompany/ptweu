...
interface CTASectionProps {
  ...
  customCTA?: string;
  /** CMS override for heading / sub / cta of the chosen type. */
  data?: { heading?: string; sub?: string; cta?: string };
  /** CMS override for the WhatsApp contact. */
...
export default function CTASection({
  ...
  const base = CONTENT[type][lang];
  const c = {
    heading: data?.heading || base.heading,
    sub: data?.sub || base.sub,
    cta: data?.cta || base.cta,
    wa: base.wa,
  };
  const waNumber = whatsapp?.number || '6281399567777';
  const waDisplay = whatsapp?.display || '+62 813 9956 7777';
