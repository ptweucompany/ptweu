...
import { ChevronRight, Factory, Zap, ShieldCheck, Star, ArrowRight } from 'lucide-react';
import { productSlugsEN } from '../../../../src/data/products';
...
const slugToId: Record<string, string> = {
  ...
  'pcc': 'pcc',
  'construction-aggregate': 'agregat',
};
...
export default async function EnProductDetailPage({ params }: Props) {
  ...
  const productSchema = {
    ...
    offers: {
      ...
      eligibleRegion: [{ '@type': 'Country', name: 'Indonesia' }, { '@type': 'Continent', name: 'Asia' }],
    },
    category: 'Industrial Mineral',
    countryOfOrigin: { '@type': 'Country', name: 'Indonesia' },
  };

  // 🧭 SEO MAXIMIZE: FAQPAGE SCHEMA (EN)
  ...
  const getPath = (id: string): any[] => {
    const p = (enT.productPages as any)[id];
    if (!p) return [];
    if (p.parent) {
    ...
    <div className="bg-white">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 pb-20 w-full">
          <nav className="mb-8 overflow-x-auto">
            <ol className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-white/50 whitespace-nowrap">
              <li><Link href="/en" className="hover:text-brand-gold transition-colors">Home</Link></li>
              <li><ChevronRight size={10} /></li>
              <li><Link href="/en/catalog" className="hover:text-brand-gold transition-colors">Catalog</Link></li>
              {path.map((step, i) => (
                <div key={step.id} className="flex items-center gap-2 contents">
                  ...
                  <li className={i === path.length - 1 ? 'text-brand-gold font-black' : ''}>
                    {i === path.length - 1 ? step.hero.title : (
                      <Link href={`/en/products/${Object.keys(slugToId).find(key => slugToId[key] === step.id)}`} className="hover:text-brand-gold transition-colors">
                        {step.hero.title}
                      </Link>
                    )}
                  </li>
          ...
          <h1 className="text-white text-4xl sm:text-7xl font-black leading-[0.9] mb-6 uppercase tracking-tighter max-w-4xl">
            {product.hero.title}
          </h1>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
            <div className="lg:col-span-7">
              <div className="mb-16">
                <h2 className="text-xs font-black text-brand-gold uppercase tracking-[0.3em] mb-6">Product Overview</h2>
              ...
              <div className="mb-16">
                <h2 className="text-xs font-black text-brand-gold uppercase tracking-[0.3em] mb-8">Technical Specifications</h2>
                <div className="overflow-hidden rounded-3xl border border-gray-100 shadow-sm">
                  <table className="w-full text-left">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-100">
                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-gray-400">Analysis Parameter</th>
                        <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-gray-400">Average Value</th>
                      </tr>
                    </thead>
              ...
              {product.size_variants && (
                <div className="mb-16">
                  <h2 className="text-xs font-black text-brand-gold uppercase tracking-[0.3em] mb-6">Size Variants</h2>
              ...
              <div className="sticky top-32 space-y-8">
                ...
                {product.derivatives && product.derivatives.length > 0 && (
                  <div className="p-8 bg-gray-900 rounded-[2rem] text-white overflow-hidden relative group">
                    ...
                    <h3 className="text-xs font-black text-brand-gold uppercase tracking-[0.3em] mb-8 relative">Derivative Products</h3>
                    <div className="space-y-3 relative">
                      {product.derivatives.map((derId: string) => {
                        const derProduct = (enT.productPages as any)[derId];
                        return (
                          <Link 
                            key={derId}
                            href={`/en/products/${Object.keys(slugToId).find(k => slugToId[k] === derId)}`}
                ...
                <div className="p-8 border border-gray-100 rounded-[2rem] bg-gray-50/50">
                  <h3 className="text-xs font-black text-brand-gold uppercase tracking-[0.3em] mb-8">Industrial Applications</h3>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="flex flex-col lg:flex-row gap-20 items-center">
            <div className="lg:w-1/2">
              <h2 className="text-xs font-black text-brand-gold uppercase tracking-[0.3em] mb-8">Modern Production Flow</h2>
              <h3 className="text-4xl md:text-5xl font-black uppercase tracking-tighter leading-[0.9] mb-12">
                How We <br /> Guarantee Quality
              </h3>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="p-12 md:p-20 bg-gray-50 rounded-[3rem] border border-gray-100 flex flex-col lg:flex-row gap-16 items-center">
            <div className="lg:w-2/3">
              <h2 className="text-xs font-black text-brand-gold uppercase tracking-[0.3em] mb-8">Absolute Quality Standards</h2>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-4xl mx-auto px-6">
          <h2 className="text-xs font-black text-brand-gold uppercase tracking-[0.3em] mb-12 text-center">Additional Information</h2>
      ...
      <CTASection
        ...
        lang="en"
        variant="dark"
        customHeading={`Empower Your Production with ${product.hero.title}`}
        customCTA="Request Quotation Now"
      />
    </div>
  );
}
