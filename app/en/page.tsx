...
export const metadata: Metadata = {
  ...
  description:
    'Leading producer and supplier of Limestone, CaCO3, Quicklime, Hydrated Lime, and PCC in North Sulawesi, Indonesia. Production 150,000 MT/month. Distribution via Port Bitung.',
  alternates: {
...
function trustBarFromBlocks(get: (k: string) => string): TrustBarContent {
  return {
    ...
    badges: get('trustbar.badges').split(',').map((s) => s.trim()).filter(Boolean),
  };
}
...
export default async function EnHomePage() {
  ...
  const g = (k: string) => pick(blocks, k, 'en');
  ...
  return (
    <main>
      ...
      <TrustBar lang="en" data={trustBarFromBlocks(g)} />
      ...
      <section className="bg-gray-50 py-12 border-y border-gray-200 text-left">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-10">
          <div>
            ...
            <p className="text-gray-600 font-medium italic border-l-4 border-[#C8A84B] pl-4">
              {g('home.midcta.desc')}
            </p>
          </div>
          <Link
      ...
      <ProductPreview
        ...
        ctaHref="/en/products"
        itemHrefBase="/en/products"
        itemCtaLabel="View Specs"
      />
