...
export default async function ContactPage() {
  ...
  const wa2Digits = digits(settings.whatsapp_2_url) || digits(settings.whatsapp_2);
  const wa2Display = settings.whatsapp_2
    ? '+62 ' + settings.whatsapp_2.replace(/^0/, '').replace(/(\d{3})(\d{4})(\d+)/, '$1 $2 $3')
    : '';
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              {wa2Digits && (
                <a href={`https://wa.me/${wa2Digits}?text=${encodeURIComponent('Hello PT WEU, I would like to know more about your industrial mineral products.')}`}
                  target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-5 bg-[#25D366]/10 border border-[#25D366]/30 rounded-2xl p-5 hover:bg-[#25D366]/15 transition-colors">
                  <div className="w-14 h-14 bg-[#25D366] rounded-xl flex items-center justify-center flex-shrink-0">
                    <svg className="w-7 h-7 text-white" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  <div>
                    <p className="text-white font-bold">WhatsApp (Second Number)</p>
                    <p className="text-[#25D366] text-lg font-bold">{wa2Display}</p>
                    <p className="text-gray-400 text-xs">Alternative — fast response</p>
                  </div>
                </a>
              )}
